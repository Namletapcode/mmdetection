...
# Load tạ pre-trained của Cascade Mask R-CNN
...
# 1. CẤU HÌNH MODEL
model = dict(
    rpn_head=dict(anchor_generator=dict(scales=[2, 4], ratios=[0.5, 1.0, 2.0], strides=[4, 8, 16, 32, 64])),
    roi_head=dict(
        bbox_head=[
...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', interval=1, max_keep_ckpts=10, out_dir='/kaggle/working/checkpoints/cascade_mask_rcnn')
)